...
const getDatabaseSchemaSchema = z.object({
  includeSystemTables: z
    .boolean()
    .optional()
    .describe("Whether to include system/internal tables (default: false, only shows user tables)"),
});

export const getDatabaseSchemaTool = tool(
  async (input) => {
    const { includeSystemTables = false } = input as z.infer<typeof getDatabaseSchemaSchema>;
...
export const listTablesTool = tool(
  async (input) => {
    const { includeSystemTables = false } = input as z.infer<typeof listTablesSchema>;
    ...
    try {
      ...
      const tables = await prisma.$queryRaw<Array<{ table_name: string }>>(query);
...
const getSampleDataSchema = z.object({
  ...
  limit: z.number().optional().describe("Number of rows to return (default: 5, max: 20)"),
});

export const getSampleDataTool = tool(
  async (input) => {
    const { tableName, limit = 5 } = input as z.infer<typeof getSampleDataSchema>;
    ...
    try {
      ...
      const tableExists = await prisma.$queryRaw<Array<{ table_name: string }>>(tableCheckQuery);
      ...
      const sampleQuery = Prisma.raw(`SELECT * FROM "${tableName}" LIMIT ${safeLimit};`);

      const rows = await prisma.$queryRaw<Array<Record<string, unknown>>>(sampleQuery);
